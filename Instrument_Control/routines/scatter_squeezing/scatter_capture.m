...
filename = '11_12_thermal_squeeze_50mv';
...
demod_rate=56.2 ;%samples per second

filter_BW=33.6 ; %Vary the phase detector bandwidth

Poll_Time=20*60 ;%Seconds
...
figure(3)
plot(V_pump(i)*1000,varx/varx_r,'b*',V_pump(i)*1000,vary/vary_r,'ro');
...
kpib('HP_E3632A',10,'setV',0,1); %Set bias to device electrode (through bias Tee)
kpib('HP_E3632A',10,'read',1);

kpib('HP_E3647A',11,'setV',0,1); %Set bias to drive electrode (through bias Tee)
kpib('HP_E3647A',11,'read',1);

kpib('HP_E3634A',5,'setI',0,1); %Set current supply to 0. The supply won't let the voltage reach Ve_max because this is 0.
%kpib('HP_E3647A',5,'setV',Ve_max,1); %Sets the max voltage o the current source
kpib('HP_E3634A',5,'read',1);
