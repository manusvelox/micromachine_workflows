...
% 'bandwidth'  sets the RBW
%           
% 'points'     sets the number of points
%
        ...
        switch command
            case 'bandwidth'
                if isequal(value,'query') || isequal(value,'?')
                    fprintf(io,'SENS:BAND:RES?');
                    retval = fscanf(io,'%f');
                    if verbose >= 2, fprintf('kpib/HP_89410A: RBW at %g\n',retval); end
                else
                    if verbose >= 2, fprintf('kpib/HP_89410A: RBW set to %g\n',value); end
                    fprintf(io,'SENS:BAND:RES %.3f',value);
                end
            case 'points'
                if isequal(value,'query') || isequal(value,'?')
                    fprintf(io,'MEM:MALL:MEAS:FPO?');
                    retval = fscanf(io,'%f');
                    if verbose >= 2, fprintf('kpib/HP_89410A: # points at %g\n',retval); end
                else
                    if verbose >= 2, fprintf('kpib/HP_89410A: # points set to %g\n',value); end
                    fprintf(io,'SWE:POIN %f',value);
                end
            case 'init'
                % do nothing
...
% 'imp'     set driven impedance. output is always 50 ohms. 
%
...
%                 retval = sscanf([data,','],'%e,');
                ...
                fprintf(io,'CALC:DATA%d?',channel);
                data = fscanf(io);
                raw = sscanf([data,','],'%e,');
                retval.x = raw(1:2:end);
                retval.y = raw(2:2:end);
%                 fprintf(io,'CALC%d:DATA?',channel);
%                 data = fscanf(io);
%                 retval.y = sscanf([data,','],'%e,');
%                 if verbose >= 2, fprintf('kpib/HP_89410A: Channel %d X & Y data downloaded.\n',channel); end
%                                
            ...
            case 'continuous' % return Status Byte

            switch value
                case {'?','query'}
                    fprintf(io,'INIT:CONT?');
                    retval = fscanf(io,'%f');
                    if verbose >= 2, fprintf('kpib_NB/KEY_N9020A: continuous %f\n',retval); end
                
                case {'OFF', 'off'}
                    if verbose >= 2, fprintf('kpib_NB/KEY_N9020A: continuous OFF\n'); end
                    fprintf(io,'INIT:CONT OFF');

                case {'ON', 'on'}
                    if verbose >= 2, fprintf('kpib_NB/KEY_N9020A: continuous ON\n'); end
                    fprintf(io,'INIT:CONT ON');

                otherwise
                if verbose >= 1, fprintf('kpib_NB/KEY_N9020A: Error, value of continuous not supported.'); end
            ...
            case {'restart'} % "pressing Meas Restart"
                fprintf(io,'INIT:IMM');
