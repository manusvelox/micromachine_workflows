function [result] = acquireDataNoTrig(boardHandle, setADC)
...
channelMask = setADC.channelSelect; 
...
if (maxSamplesPerRecord > 0) && (samplesPerChannel > maxSamplesPerRecord)
...
bufferCount = uint32(min([16 buffersPerAcquisition+2])); % PREVIOUSLY: uint32(16); allocated fixed number of buffers, even if not needed
...
fid = -1;
if saveData
    fid = fopen(setADC.fileName, 'w');
    if fid == -1
        fprintf('Error: Unable to create data file\n');        
    end
end

% TODO: Select AutoDMA flags as required from provided data
...
admaFlags = ADMA_EXTERNAL_STARTCAPTURE + ADMA_FIFO_ONLY_STREAMING;

if setADC.trigger
    admaFlags = admaFlags + ADMA_TRIGGERED_STREAMING;
else
    admaFlags = admaFlags + ADMA_CONTINUOUS_MODE;
end
...
while ~captureDone
    ...
    if setADC.triggerOutReq
        %fprintf('Buffer Number %2.0f\n', buffersCompleted)
       if floor(buffersCompleted + 1) == floor(setADC.trigOutBufferNum)
            eval(setADC.triggerOutFunction);
            fprintf('Trigger Command Evaluated\n');
       end
    end
    ...
    if bufferFull
        % Create trigger output at desired loop iteration
